...
export class AdminProductsComponent implements OnInit {
  ...
  getFirebaseCategories(): void {
    this.categorySubscription = this.service.getCategories().subscribe(data => {
  ...
  getFirebaseProducts(): void {
    this.productSubscription = this.service.getProducts()
      .subscribe(data => {
        this.cloudProducts = data.map(
          product => {
  ...
  confirmDelete(): void {
    this.service.deleteProduct(this.deletedProduct).then(
      () => {
        this.getFirebaseProducts();
        this.openSnackBar('product was successfully deleted😊', 'close', 'snackbar');
        this.modalRef.hide();
      }
    );
  }
  ...
  filterProducts(event) {
    if (event.value === 'all') {
      ...
      this.firestore.firestore.collection('products').where(
        'category.nameENG', '==', event.value)
        .get()
        .then((querySnapshot) => {
          this.cloudProducts = [];
